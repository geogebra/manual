...
* xref:/en_Special__Upload?wpDestFile=İnterval_left_png.adoc[16px]
xref:/en_Special__Upload?wpDestFile=İnterval_between_png.adoc[16px]
xref:/en_Special__Upload?wpDestFile=İnterval_right_png.adoc[16px] Birikimli olasılığı (örneğin _P(x ≤ X)_, _P(x ≥ X)_)
...
xref:/Grafik_Görünümü.adoc[Graphics View] (GeoGebra Desktop).
