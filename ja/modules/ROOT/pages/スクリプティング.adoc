...
* Javascript のイベントリスナーが発動した時(xref:/リファレンス：JavaScript.adoc[リファレンス：JavaScript] を参照)
...
`++ggbApplet.method_name(parameter,..,parameter)++` という方法で呼び出すことができる．これらのメソッドのリストについてはxref:/リファレンス：JavaScript.adoc[リファレンス：JavaScript]を参照のこと．
...
イベントリスナーは，名前の再定義，消去，クリアなどの動作に対して行うことができる．xref:/リファレンス：JavaScript.adoc[リファレンス：JavaScript]にすべてのリストが説明されている．
