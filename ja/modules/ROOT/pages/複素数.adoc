...
GeoGebra
は複素数を直接サポートしていないが，xref:/点とベクトル.adoc[点とベクトル]を用いて平面上の複素数を表したり，複素数を使って代数演算を行ったりできる．
...
で任意の点を複素数として表示することができる．点の image:16px-Menu-options.svg.png[Menu-options.svg,width=16,height=16] 
 xref:/プロパティダイアログ.adoc[プロパティダイアログ] を開き，_数式_ タブの 座標のリストから _複素数_ を選択する．

====

虚数単位 ί は，xref:/入力バー.adoc[入力バー]の記号ボックスから選択するか，[.kcode]#Alt# + [.kcode]#i#
(Mac では [.kcode]#Option# + [.kcode]#i#)で入力できる．

* image:16px-Menu_view_algebra.svg.png[Menu view algebra.svg,width=16,height=16] _数式ビュー_ では，i が定義されていない場合に限り，i は点 (0, 1) または複素数 0 + 1i として認識される（例：q = 3 + 4i）．
* image:16px-Menu_view_cas.svg.png[Menu view cas.svg,width=16,height=16]
xref:/CASビュー.adoc[CASビュー] では，i は文字（変数）として扱われるため，虚数単位としては使用できない．
