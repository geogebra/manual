...
ApplyMatrix( <xref:/行列.adoc[行列]>, <xref:/幾何オブジェクト.adoc[オブジェクト]> )::
  オブジェクト _O_ 上の点 _P_ が次のように写るように，オブジェクト _O_ を変換する：

* _P_ が _2次元_ 点であり，_M_ が2×2行列である場合，点 _P_ を点 _M*P_ に写す

* _P_ が __2次元__点であり，_M_ が3×3行列である場合，P(x,y)より _M(x,y,1) =(x’, y’, z’)_ として，点 _P_ を__(x/z,
y/z)__に写す

* _P_ が _3次元_ 点であり，_M_ が3×3行列である場合，点 _P_ を点 _M*P_ に写す

* _P_ が _3次元_ 点であり，_M_ が2×2行列である場合，_M_ = stem:[\begin{pmatrix}a&b\\ c&d \end{pmatrix}] から _N_ =
stem:[\begin{pmatrix}a&b&0\\ c&d&0\\0&0&1 \end{pmatrix}] を作り，点 _P_ を点 _N*P_ に写す
...
このコマンドは四角形やxref:/tools/画像の挿入.adoc[画像]でも機能する．
