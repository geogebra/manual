...
f(x) = 3 / x¹ + 4 / x² + 5 / x³  (n'est pas une écriture de polynôme, noter l'exposant 1)
et pourtant Coeeficients(f) opère et retourne {3, 1, 4, 2, 5, 3} 

Somme(Séquence(l1(j) x^(-l1(j + 1)), j, 1, Longueur(l1), 2))  permettra ici (somme avec 2 +) de passer à une écriture avec exposants négatifs
