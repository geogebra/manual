...
La fonction doit être continue et définie sur son ensemble de définition classique, (des restrictions ne doivent pas lui être appliquées) et n'avoir, sur l'intervalle, qu'un seul _maximum local_  et pas de minimum local.


Pour les fonctions polynomiales, il est conseillé d'utiliser la commande xref:/commands/Extremum.adoc[Extremum]
====
...
* `++Max(x^2,-1,2)++` retourne le point _(2,4)_
* `++Max(-x^2,-1,2)++` retourne le point _(0,0)_
