...
* `++EnPolaires(u)++` retourne un nouveau vecteur _(1.41 ; 45°)_ si u = stem:[ \begin{pmatrix}1 \\ 1 \end{pmatrix} ]  ;
...
* `++v:= EnPolaires(1 + sqrt(3) * ί)++` retourne un vecteur v  = stem:[ \begin{pmatrix}2 \\ \frac{\pi}{3} \end{pmatrix} ]  ;
...
`++v:=EnPolaires(u)++` ou `++EnPolaires({1,1})++` retourne le vecteur ? point ? stem:[ \left(\sqrt{2}; \frac{\pi }{4} \right)] si u = stem:[\begin{pmatrix}1 \\ 1 \end{pmatrix}] .
