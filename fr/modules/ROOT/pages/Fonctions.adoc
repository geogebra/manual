...
le mode /Fichier:Tool_Move_gif.adoc[image:Tool_Move.gif[Tool Move.gif,width=32,height=32]]
xref:/tools/Déplacer.adoc[Déplacer]). Les autres xref:/commands/Commandes_Transformations.adoc[Commandes
Transformations] peuvent être aussi appliquées aux courbes représentatives, mais dans la plupart des cas, la courbe
image n'est plus une courbe représentative de fonction.
