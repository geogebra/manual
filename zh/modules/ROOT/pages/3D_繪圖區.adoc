...
**3D 繪圖區**會放在 xref:/代數區.adoc[image:16px-Menu_view_algebra.svg.png[Menu view
...
xref:/代數區.adoc[image:16px-Menu_view_algebra.svg.png[Menu view algebra.svg,width=16,height=16]]
...
**3D 繪圖區**建立的所有物件，會同時在 xref:/代數區.adoc[image:16px-Menu_view_algebra.svg.png[Menu view
...
用滑鼠點選**xref:/tools/球面（球心、一點）.adoc[球面（球心、一點）]工具**，並分別在
...
除了可運用在其它**xref:/視區.adoc[視區]**的各種xref:/Category__指令.adoc[*指令*]，**3D 繪圖區**有專用的 *3D 指令*。
...
使用 xref:/tools/移動.adoc[image:16px-Mode_move.svg.png[Mode move.svg,width=16,height=16]]
...
**xref:/tools/旋轉_3D_繪圖區.adoc[旋轉 3D 繪圖區]工具**並用您的指向裝置拖曳
image:16px-Perspectives_algebra_3Dgraphics.svg.png[Perspectives algebra 3Dgraphics.svg,width=16,height=16] **3D
繪圖區**的背景，即可旋轉整個坐標系。
...
*xref:/tools/面對指定面.adoc[面對指定面]工具*，可將坐標系的視角轉至面對指定的物件。
