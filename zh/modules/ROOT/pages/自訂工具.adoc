...
tool can be used both with the mouse and as a command in the xref:/指令列.adoc[Input Bar]. All tools are
...
xref:/Execute_Command.adoc[Execute Command].
...
xref:/工具功能表.adoc[Tools Menu].

== Saving custom tools

When you save the construction as GGB file, all custom tools are stored in it. To save the tools in separate file(s) use
the xref:/Tool_Manager_Dialog.adoc[Tool Manager Dialog] (option Manage Tools from xref:/工具功能表.adoc[Tools Menu]).
...
New.png,width=16,height=16] New Window from the xref:/檔案功能表.adoc[File Menu]), or open GeoGebra on another day, your
...
Save.png,width=16,height=16] Save Settings from the xref:/選項功能表.adoc[Options Menu]. From now on, your customized
...
xref:/工具功能表.adoc[Tools Menu]. Then, select your custom tool from the list of tools on the left hand side of the
...
time. Just select item image:Menu_Open.png[Menu Open.png,width=16,height=16] Open from the xref:/檔案功能表.adoc[File
