...
graphics.svg,width=16,height=16]]**xref:/繪圖區.adoc[繪圖區]**旁邊。上方所顯示的工具列會因為你目前所在的**視區**不同而顯示不同的**工具列**，若你正在編輯**運算區**，則上方所顯示的工具列是「xref:/En__CAS_Tools.adoc[*運算區工具列*]」；若正在編輯*繪圖區*，則會切換為「xref:/En__Graphics_Tools.adoc[*繪圖工具列*]」。「復原」、「重複」按鈕放置在視窗的右上角。
...
cas.svg,width=16,height=16]**運算區**是xref:/格局.adoc[*運算格局*]的一部分，但您仍可根據自己的需求來xref:/En__GeoGebra_5_0_Desktop_vs_Web_and_Tablet_App.adoc[調整]
GeoGebra 介面的外觀。
...
*備註：*
除了使用這些快捷鍵之外，您也可使用<<運算工具列,*運算區工具列*>>上的xref:/En__CAS_Tools.adoc[*工具*]。
...
cas.svg,width=16,height=16]**運算區**有xref:/En__CAS_Commands.adoc[*運算區專用的指令*]，讓您能在
image:16px-Menu_view_cas.svg.png[Menu view
...
[NOTE]
====
詳細清單請參閱「*xref:/En__CAS_Commands.adoc[運算區指令]*」。
...
cas.svg,width=16,height=16]**運算區**支援xref:/En__CAS_View_Supported_Geometry_Commands.adoc[某些**幾何指令**]的代數運算。

====

== [#運算工具列]####[#.E9.81.8B.E7.AE.97.E5.B7.A5.E5.85.B7.E5.88.97]##運算工具列##

xref:/En__CAS_Tools.adoc[*運算工具列*]提供許多*xref:/En__CAS_Tools.adoc[運算工具]*，讓您能進行代數運算或數值運算。只要先輸入資料，再用滑鼠點擊對應的**運算工具**即可。
...
[NOTE]
====
詳細**工具**清單請參閱「*xref:/En__CAS_Tools.adoc[運算區工具]*」。
...
* *複製為LaTeX*
：複製本列內容到電腦剪貼簿，讓您可在別處貼上，例如：xref:/En__Text.adoc[文字]物件。（僅適用於桌機版）
...
* *複製為 LaTeX*：以 LaTex
格式複製儲存格內容到電腦剪貼簿，如此便可轉貼到某個xref:/En__Text.adoc[文字]物件或
LaTeX 編輯器。
* *複製為 LibreOffice 公式*：以 LibreOffice 公式格式複製儲存格內容到電腦剪貼簿，如此便可轉貼到某個文書處理檔案。
* *複製為圖片*：以 PNG
格式複製儲存格內容到電腦剪貼簿，如此便可轉貼到某個xref:/En__Image_Tool.adoc[圖形]物件或其他文件。
...
graphics.svg,width=16,height=16]**xref:/En__Graphics_View.adoc[繪圖區]**中利用
