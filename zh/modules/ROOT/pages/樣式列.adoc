...
**樣式列**可讓您簡易且快速地更改**xref:/視區.adoc[視區]**或xref:/En__Objects.adoc[物件]的一些基本屬性。在桌機版要打開或關閉*樣式列*，點擊**xref:/視區.adoc[視區]**名稱左邊的小三角形；而在線上版或平板版，則是點擊*樣式列按鈕*。進入*xref:/En__Properties_Dialog.adoc[屬性對話窗]*，能調整更多屬性設定。

== [#視區的樣式列選項]####[#.E8.A6.96.E5.8D.80.E7.9A.84.E6.A8.A3.E5.BC.8F.E5.88.97.E9.81.B8.E9.A0.85]##**視區**的樣式列選項##

每個**xref:/視區.adoc[視區]**有專屬的**樣式列**選項。在xref:/En__GeoGebra_5_0_Desktop_vs_Web_and_Tablet_App.adoc[線上版或平板版]，根據您所選取的**視區**，可以使用下列任一個**樣式列按鈕**來開啟**樣式列**，將會顯示該**視區**最常用的選項。
...
capturing.svg,width=16,height=16]**選擇吸附模式**：您可選擇**自動**、*吸附至格線*、*固定於格線*、**關閉**等不同模式。更進一步資訊請見「xref:/En__Point_Capturing.adoc[點吸附]」。
* image:16px-Menu-options.svg.png[Menu-options.svg,width=16,height=16]**開啟設定對話窗**：開啟**繪圖區**或所選物件的**xref:/En__Properties_Dialog.adoc[屬性對話窗]**（僅適用於線上版或平板版）。
...
繪圖區**或所選物件的*xref:/En__Properties_Dialog.adoc[屬性對話窗]*（僅適用於線上版或平板版）。
* image:16px-Stylingbar_dots.svg.png[Stylingbar
dots.svg,width=16,height=16]**顯示視區**：您可在視窗中關閉或開啟其他**xref:/視區.adoc[視區]**（僅適用於線上版或平板版）。
...
objects.svg,width=16,height=16]顯示／隱藏*輔助物件*：點選即可切換是否在**代數區**顯示xref:/En__Free_Dependent_and_Auxiliary_Objects.adoc[*輔助物件*]。
...
** *相依性*：物件會分成xref:/En__Free_Dependent_and_Auxiliary_Objects.adoc[*自變物件*]和xref:/En__Free_Dependent_and_Auxiliary_Objects.adoc[*應變物件*]兩類。若您沒有配合其他任何已存在的物件，來建立一個新物件，此物件將被視為*自變物件*（Free
Object）。反之，則被視為**應變物件**（Dependent Object）。
** *物件類別*：預設情況下，物件會依照**物件類別**分類（例如：角度、直線、點），且會以字母順序排序。
** *圖層*：物件依照它們所在的圖層分類。更進一步資訊請參閱「*xref:/En__Layers.adoc[圖層]*」。
** *作圖順序*：依照作圖順序來排序。
* image:16px-Stylingbar_dots.svg.png[Stylingbar
dots.svg,width=16,height=16]**顯示視區**：您可在視窗中關閉或開啟其他**xref:/視區.adoc[視區]**（僅適用於線上版或平板版）。
...
* image:16px-Menu-options.svg.png[Menu-options.svg,width=16,height=16]**設定對話窗**：開啟**xref:/En__Properties_Dialog.adoc[設定對話窗]**（僅適用於線上版或平板版）。
* image:16px-Stylingbar_dots.svg.png[Stylingbar
dots.svg,width=16,height=16]**顯示視區**：您可在視窗中關閉或開啟其他**xref:/視區.adoc[視區]**（僅適用於線上版或平板版）。
...
根據您所選取的xref:/En__Tools.adoc[*工具*]或物件，**樣式列**上會顯示相對應的一組按鈕，可用來更改以下屬性：
...
* image:16px-Menu-options-labeling.svg.png[Menu-options-labeling.svg,width=16,height=16]**選擇標籤模式**：您可選擇以下的xref:/En__Labels_and_Captions.adoc[*標籤模式*]
** *隱藏*：不顯示標籤。
** *名稱*：僅顯示物件名稱（例如：_A_）。
** *名稱與數值*：顯示物件名稱及其數值（例如：_A = (1, 1)_）。
** *數值*：僅顯示物件數值（例如：_(1, 1)_）。
* image:16px-Stylingbar_caption.svg.png[Stylingbar
caption.svg,width=16,height=16]**標籤文字**：除了物件的名稱和數值，您可於**xref:/En__Properties_Dialog.adoc[屬性對話窗]**進行設定，顯示自訂的xref:/En__Labels_and_Captions.adoc[標籤文字]（例如：多個物件皆顯示相同標籤）。
* image:Pin.png[Pin.png,width=16,height=16]**螢幕上的絕對位置**：您可在螢幕上固定某個物件（例如：文字物件），則此物件不受「xref:/En__Move_Graphics_View_Tool.adoc[移動（3D）繪圖區]」或「螢幕縮放」影響（僅適用於桌機版）。
* image:16px-Menu-options.svg.png[Menu-options.svg,width=16,height=16]**設定對話窗**：開啟**xref:/En__Properties_Dialog.adoc[設定對話窗]**（僅適用於線上版或平板版）。
* image:16px-Stylingbar_dots.svg.png[Stylingbar
dots.svg,width=16,height=16]**顯示視區**：您可在視窗中關閉或開啟其他**xref:/視區.adoc[視區]**（僅適用於線上版或平板版）。
