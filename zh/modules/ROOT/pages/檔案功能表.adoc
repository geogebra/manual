...
GGB）、xref:/En__Open_Dialog_Style_Templates.adoc[樣式範本]（副檔名為 GGB）、GeoGebra
工具檔（副檔名為 GGT）或是動態網頁（由 GeoGebra 產生的 HTM 或 HTML 檔）。

您也可在現存的某個 GGB 檔案中xref:/En__Open_Dialog_Insert_File.adoc[插入另一個 GGB
檔案]。
...
讓您直接上傳圖檔至
https://www.geogebra.orgxref:/[GeoGebra]，另請參閱「xref__/En__Export_Worksheet_Dialog.adoc[匯出網頁
(html) ...]」。
...
xref:/En__Export_Worksheet_Dialog.adoc[匯出網頁 (html) ...]
* image:Image-x-generic.png[Image-x-generic.png,width=16,height=16]
xref:/En__Export_Graphics_Dialog.adoc[匯出圖檔 (png, eps) …]
* image:Edit-copy.png[Edit-copy.png,width=16,height=16] 複製到剪貼簿
* ... 以及xref:/En__Export_to_LaTeX_(PGF_PSTricks)_and_Asymptote.adoc[其他]
...
此選項將開啟xref:/繪圖區.adoc[繪圖區]的xref:/En__Print_Preview_Dialog.adoc[預覽列印對話窗]。您可指定標題、作者、日期以及輸出的比例（以
cm 為單位）。
