...
在預設的情況下，**工具列**會位於視窗的上方，或是**xref:/功能表.adoc[功能表]**的下方（桌機版）。**工具列**由多個**工具箱**所組成，每一個工具箱就像個抽屜，收納數個相關的*xref:/En__Tools.adoc[工具]*。
...
在 GeoGebra
桌機版，您可利用**xref:/檢視功能表.adoc[檢視]**選單的image:16px-Menu-perspectives.svg.png[Menu-perspectives.svg,width=16,height=16]xref:/En__GeoGebra_5_0_Desktop_vs_Web_and_Tablet_App.adoc[*版面*]設定來改變**工具列**的位置。

=== 在 GeoGebra Applet 調整工具列

可使用 xref:/En__Reference__Applet_Parameters.adoc[_customToolBar_]
