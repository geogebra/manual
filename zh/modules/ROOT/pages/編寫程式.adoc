...
* 最後，調整 a 值就可以改變它的xref:/En__Reference__Colors.adoc[顏色]。
...
有關顏色的名稱與編碼，請參閱：xref:/En__Reference__Colors.adoc[完整的顏色列表]
...
xref:/En__Reference__JavaScript.adoc[ggbApplet 物件方法]列表。
...
xref:/En__Reference__JavaScript.adoc[registerAddListener]()
...
除了指定「新增物件」的觸發程序外，我們也可以指定諸如「改名」、「刪除」、「清除全部物件」等等事件的觸發程序，完整的說明請看xref:/En__Reference__JavaScript.adoc[這裡]。
...
* 請看示範網頁：https://www.geogebra.org/source/program/applet/geogebra_applet_java2javascript.htm[ggbApplet 與
JavaScript 之間的互動]。
