...
xref:/A_Geometria_Nézet_beállításai.adoc[Customizing the Graphics View] for
details.
...
coordinate axes_ and the _grid_ or change the xref:/Beállítások_menü.adoc[point
...
A second graphics view may be opened using the xref:/Nézet_menü.adoc[View Menu].
...
xref:/Parancsok.adoc[Commands] appear in the active graphics view. For each
