...
xref:/Customizing_the_Graphics_View.adoc[Customizing the Graphics View] for
details.
...
coordinate axes_ and the _grid_ or change the xref:/Options_Menu.adoc[point
...
A second graphics view may be opened using the xref:/View_Menu.adoc[View Menu].
...
xref:/Commands.adoc[Commands] appear in the active graphics view. For each
