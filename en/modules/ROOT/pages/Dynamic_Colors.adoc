...
*Note:* You may also xref:/Animation.adoc[animate] the sliders with different speeds, in order to see the colour of the
polygon change automatically.
...
*Example:* To make a point _A_ go through all the colours of the rainbow when moved left and right, switch to _HSV_
mode, then set _Saturation_ and _Value_ to 1, and _Hue_ to `x(A)`.
