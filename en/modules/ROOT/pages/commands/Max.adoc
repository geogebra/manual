...
Max( <Function>, <Start x-Value>, <End x-Value> )::
  ...
  continuous, defined over its natural domain (i.e. no domain restrictions are applied), and have only one _local_ maximum point in the interval (and no local minimum).
