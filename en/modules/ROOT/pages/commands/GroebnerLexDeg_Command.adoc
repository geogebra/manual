...
*Example:* `GroebnerLexDeg({x^3 - y - 2, x^2 + y + 1})` yields \{stem:[ -y^\{2} + x - 3 y - 3, -x y - x - y - 2, x^\{2}
+ y + 1]}.
...
*Example:* `GroebnerLexDeg({x^3 - y -2, x^2 + y + 1},{y, x})` yields \{stem:[ x^\{2} + y + 1, -y x - y - x - 2, y^\{2} +
3 y - x + 3 ]}.
