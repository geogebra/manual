...
Gerade für interaktive Arbeitsblätter mit xref:/Skripting.adoc[Scripting] können Aktionsobjekte sehr praktisch sein.
...
Eingabefelder dienen als Texteingabe für xref:/Skripting.adoc[Skripte]. Das Skript wird ausgelöst, wenn der Text im
...
andere Objekte aufgerufen werden (z.B. durch ein xref:/tools/Bild.adoc[Bild]). Verwendet man dafür
