...
Per ottenere la lista di tutti gli estremi locali del polinomio, racchiudere il comando in una coppia di
parentesi graffe, come ad es. `{Estremo(x³ + 3x² - 2x + 1)}`.
...
La funzione deve essere continua nell'intervallo specificato, altrimenti potrebbe essere calcolato un falso
estremo nell'intorno di ciascun punto di discontinuità.
...
`Assumi(0 < x < 20, Estremo(15/2 * sin( 2/15*pi * x) + 56/5))` restituisce i flessi locali nell'intervallo
indicato _stem:[ \left\\{ \left(\frac\{15}\{4}, \frac\{187}\{10} \right), \left(\frac\{45}\{4}, \frac\{37}\{10} \right),
