...
xref:/tools/Strumento_Mostra_/_Nascondi_etichetta.adoc[strumento Mostra / Nascondi etichetta] e selezionare l'oggetto di
...
Per valore di un punto si intendono le relative coordinate, mentre il valore di una funzione è la relativa
equazione.
...
Sia A un punto, di coordinate (1,2). Impostando la legenda come "Il punto %n ha coordinate %v" verrà
visualizzato "Il punto A ha coordinate (1,2)"
...
Se si desidera un testo matematicamente elegante conviene utilizzare LaTeX nelle legende. Ad esempio, per
visualizzare una frazione del tipo stem:[ \frac\{a}\{b} ], digitare `$\frac{a}{b}$` nel campo _Legenda_
