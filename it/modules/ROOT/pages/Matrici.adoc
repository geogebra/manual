...
In GeoGebra, \{\{1, 2, 3}, \{4, 5, 6}, \{7, 8, 9}} rappresenta la matrice 3x3 stem:[\begin\{pmatrix}1&2&3\\
...
Digitare `LaTeX[{{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}]` nella barra di inserimento per visualizzare la
corrispondente matrice in formato LaTeX.
...
\{\{1, 2}, \{3, 4}, \{5, 6}} * \{\{1, 2, 3}, \{4, 5, 6}} restituisce la matrice \{\{9, 12, 15}, \{19, 26,
33}, \{29, 40, 51}}.
...
Per le trasformazioni affini in cui si utilizzano coordinate omogenee: (x, y, 1) denota un punto e (x, y, 0) un
vettore. L'esempio precedente è quindi equivalente a: `{{1, 2, 3}, {4, 5, 6}, {0, 0, 1}} * {1, 2, 1}`.
