...
È inoltre possibile ottenere le derivate di una funzione _f_(_x_) precedentemente definita digitando _f'_ (x) o
_f"_ (x), e così via.
...
Dopo avere definito la funzione `f(x) = 3 x^3 – x^2`, digitando `g(x) = cos(f' (x + 2))` si ottiene la
funzione composta _g_.
