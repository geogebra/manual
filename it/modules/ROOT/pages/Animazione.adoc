...
Quando si avvia l'animazione di un numero, di un angolo libero o di un punto vincolato, viene visualizzato
nell'angolo in basso a sinistra della image:16px-Menu_view_graphics.svg.png[Menu view graphics.svg,width=16,height=16]
_vista Grafici_ un pulsante per gestire l'animazione, che consente di image:18px-Nav_pause_circle.svg.png[Nav pause
...
Quando viene avviata un'animazione automatica, le funzionalità di GeoGebra restano invariate. In questo modo è
possibile apportare modifiche alla costruzione anche durante l'animazione.
...
Se le coordinate di un punto dipendono da un numero _t_ come in P = (2 t, t), il punto si muoverà lungo una
retta quando _t_ cambia con continuità.
...
Un punto appartenente a una retta può essere mosso lungo la retta anche premendo i tasti [.kcode]#+# o
[.kcode]#–# .
