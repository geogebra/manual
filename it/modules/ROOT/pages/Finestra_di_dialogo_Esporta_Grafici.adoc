...
Quando si esporta in formato SVG e PDF è disponibile l'opzione di esportazione del testo come testo modificabile
o come forma: ciò significa che nel primo caso il testo viene salvato come testo puro (che può essere modificato ad es.
con InkScape), mentre nel secondo caso il testo viene salvato come curva di Bézier (così il testo verrà visualizzato
