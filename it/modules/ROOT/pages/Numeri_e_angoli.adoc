...
Se la variabile _e_ non è già stata utilizzata come nome di un oggetto esistente, verrà automaticamente
riconosciuta all'interno delle espressioni inserite come numero di Nepero.
...
Per gli angoli dipendenti è possibile specificare se consentire anche angoli concavi, selezionando la relativa
opzione nella scheda _Generali_ della image:16px-Menu-options.svg.png[Menu-options.svg,width=16,height=16]
...
I numeri e gli angoli xref:/Oggetti_liberi,_dipendenti_e_ausiliari.adoc[liberi] possono essere visualizzati con slider
