...
PodrobnostiDukazu( <Pravdivostní hodnota> )::
  Poskytne určité detailnější informace o výsledku automatického důkazu pravdivosti vloženého výrazu s pravdivostní hodnotou  (tzv. booleovského výrazu nabývajícího hodnoty _true_ nebo _false_).

Normálně GeoGebra rozhoduje o pravdivosti xref:/Pravdivostní_hodnoty.adoc[booleovského výrazu], tj. zda je jeho hodnota _true_ nebo _false_, pomocí numerického výpočtu. Příkaz _PodrobnostiDukazu_ však pro vyhodnocení toho, zda má vložený booleovský výraz hodnotu _true_ nebo _false_, používá https://en.wikipedia.org/wiki/Symbolic_computation[symbolické metody] výpočtu. Zabývá se tak obecnou platností výrazem postihnuté vlastnosti. This command works like the
