...
* Pro použití tohoto příkazu v objektu _Text_ je třeba zaškrtnout práci v prostředí *_LaTeX vzorec_* v
 image:16px-Menu-options.svg.png[Menu-options.svg,width=16,height=16] xref:/Dialog_Vlastnosti.adoc[Properties Dialog].
...
* Viz také xref:/commands/ZlomekText.adoc[ZlomekText] a příkaz xref:/commands/VedeckyText.adoc[ScientificText].
