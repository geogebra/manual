...
Pro změnu rozložení celého okna GeoGebry můžete popsat požadované uspořádání _náhledů_ pomocí výrazu. Jednotlivé _náhledy_ jsou v něm reprezentovány písmeny: horizontální uspořádání _náhledů_ je definováno pořadím písmen vedle sebe, jejich vertikální uspořádání je dosaženo užitím symbolu dělení `++ /++`.
