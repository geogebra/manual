...
Kliknutím na volné místo nákresny v režimu xref:/Nákresna.adoc[Grafický pohled] se vytvoří posuvník, kterým lze
...
Umístění posuvníku v xref:/Nákresna.adoc[Grafickém pohledu] může být buď absolutní (to znamená, že poloha
posuvníku v okně není ovlivňována přibližováním či oddalováním nákresny, ale vždycky zůstává ve viditelné části
xref:/Nákresna.adoc[Grafického pohledu]), nebo relativní vzhledem k soustavě souřadnic (tj. při přibližování
...
posuvník prostým zobrazením tohoto čísla nebo úhlu v xref:/Nákresna.adoc[Grafickém pohledu] (viz
